...
class TransporterPickUpConfirmSignatureView extends ConsumerStatefulWidget {
  const TransporterPickUpConfirmSignatureView({
  ...
  @override
  _TransporterPickUpConfirmSignatureViewState createState() =>
      _TransporterPickUpConfirmSignatureViewState();
}

class _TransporterPickUpConfirmSignatureViewState
    extends ConsumerState<TransporterPickUpConfirmSignatureView> {
          ...
          children: [
            ...
            AppButton.primary(
              onPressed: () async {
                ...
                await ref.read(signingServiceProvider).transporterPickUpSigning(
                      producerPickUp.transporterId!,
                      signature,
                      producerPickUp.id,
                      latitude,
                      longitude,
                ...
                await ref.read(producerServiceProvider).updatePickUpStatus(
                      producerPickUp.id,
                    );
                ...
                await showAdaptiveDialog(
                  ...
                  builder: (context) {
                    return AlertDialog.adaptive(
                      title: const Text('Signature confirmée'),
                      content: const Text(
                        'La signature a été confirmée avec succès.',
                      ),
                      actions: [
                        CupertinoButton(
                          onPressed: () {
                ...
                ref.invalidate(producerPickUpFutureProvider);
